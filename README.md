...
newer -- every bump goes through `uv version --bump`, which older releases
reject -- and a static `[project].version` in `pyproject.toml`. Automatic version selection and

...
def test_a_rejected_token_is_reported():
    with pytest.raises(VommitError, match="PyPI could not verify this token"):
...
def test_an_unexpected_response_is_not_accepted():
    with pytest.raises(VommitError, match="200"):
        auth.verify_token(TOKEN, post=FakeIndex(200).post)
...
def test_a_405_is_not_taken_as_approval():
    # what an empty request gets; it cannot establish token validity
    with pytest.raises(VommitError, match="405"):
        auth.verify_token(TOKEN, post=FakeIndex(405).post)
